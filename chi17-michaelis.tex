...
Findings from human-robot interaction studies also provide several guidelines for increasing user engagement in human-robot interaction, including the robot making direct eye contact when speaking to its user \cite{Mutlu:2011} as well as averting it's gaze while it is ``thinking'' \cite{Andrist:2014}; providing tailored recommendations for content \cite{Lim:2013}; and making reference to previous interactions \cite{Leite:2009}. By incorporating several of these design guidelines into its behavior, we believe that a learning companion robot may serve as a powerful tool for supporting interest and engagement in reading for young readers. 
